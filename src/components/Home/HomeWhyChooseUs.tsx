...
const HomeWhyChooseUs: React.FC = () => (
  <section className="w-full py-12 bg-white">
    <div className="w-full px-4 sm:px-6 mx-auto bg-gray-100 rounded-2xl md:px-12 py-8 flex flex-col md:flex-row items-center gap-10">
      {/* Left: Image */}
      <Reveal variant="left" className="flex-1">
        <div className="flex-1 flex justify-center">
          <img
            ...
            alt="Why Choose Us"
            className="rounded-2xl object-cover w-full max-w-xl h-64 sm:h-80 md:h-[36rem]"
          />
        </div>
      </Reveal>
      {/* Right: Content */}
      <div className="flex-1 w-full flex flex-col items-center md:items-start text-center md:text-left">
        <h2 className="text-3xl md:text-4xl font-bold mb-8 w-full text-center md:text-left">
        ...
        <div className="flex flex-col gap-4 md:gap-6 w-full">
          {features.map((feature, idx) => (
            <Reveal key={feature} variant="right" delayMs={idx * 120}>
              <div
                className="group flex items-center bg-white rounded-xl shadow px-6 py-4 gap-4 w-full anim-card-lift"
              >
